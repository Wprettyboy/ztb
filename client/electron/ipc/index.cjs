...
const { registerProcurementAgentIpc } = require('./procurementAgentIpc.cjs');
...
const { createProcurementAgentService } = require('../services/procurementAgentService.cjs');
...
function registerWorkspaceDatabaseServices({ app, configStore, aiService, fileService, updateStatus }) {
  ...
  const taskService = createTaskService({ aiService, technicalPlanStore, knowledgeBaseService });
...
function registerIpcHandlers({ app, mainWindow, gpuStartupState = {}, gpuTrialArg = '--yibiao-trial-hardware-acceleration', forceDisableGpuArgs = [] }) {
  const configStore = createConfigStore(app);
  const aiService = createAiService({ app, configStore });
  const fileService = createFileService({ app, configStore });
  const exportService = createExportService({ configStore });
  const procurementAgentService = createProcurementAgentService({ app, configStore, aiService });
  ...
  registerProcurementAgentIpc({ procurementAgentService });
